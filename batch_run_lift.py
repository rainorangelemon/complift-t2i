...
for prompt, combination in zip(prompts, combination_types):
    ...
    for seed_to_generate in range(64):
        if (os.path.exists(LiftConfig.output_path / f"{prompt}" / f"{seed_to_generate}.png")) or \
            (os.path.exists(Path(str(LiftConfig.output_path)+"_invalid") / f"{prompt}" / f"{seed_to_generate}.png")):
            continue
        else:
    ...
    command = f"python run_lift.py --prompt \"{prompt}\" --seeds {seeds} --token_indices {token_to_combine} --prompts {prompts}"
    print(command)
