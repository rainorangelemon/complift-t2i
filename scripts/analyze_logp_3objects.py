...
for i in range(10):
    lift_results = torch.load(f"outputs/3objects_lift/a turtle and a ball in a box/{i}_lift_results.pt", weights_only=True)
...
scores_path = "outputs/3objects_lift/a turtle and a ball in a box/scores.pt"
if os.path.exists(scores_path):
    # Load cached scores
    cached_scores = torch.load(scores_path, weights_only=True)
    score_composed_all = cached_scores["composed"]
    score_turtle_all = cached_scores["turtle"]
    score_ball_all = cached_scores["ball"]
    score_box_all = cached_scores["box"]
    ...
    for img_idx in tqdm(range(10)):
        score_results = calculate_score(latest_latents[[img_idx]],
                                     callback,
                                     pipeline,
                                     prompts=["a turtle and a ball in a box", "a turtle", "a ball", "a box", ""],
...
class VisualizationPlotter:
    ...
    def plot_heatmap(self, ax, heatmap: torch.Tensor, title: str, row_idx: int):
        ...
        display_title = f"$\it{{Lift}}$ for {obj_name}\n" if row_idx == 0 else ""
    ...
    def create_visualization(self, data: dict):
        ...
        for plot_idx, img_idx in enumerate(tqdm(indices_to_plot)):
            ...
            for obj_idx, obj_name in enumerate(['ball', 'turtle', 'box']):
                ...
                self.plot_heatmap(ax[plot_idx][obj_idx + 2], heatmap, f"$\it{{Lift}}$ for {obj_name.capitalize()}", plot_idx)
        ...
        plt.tight_layout()
        plt.savefig("t2i_example.pdf")

# Usage example:
config = VisualizationConfig(
    specific_indices=[2, 3, 7, 8],
